        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              ...
              {tab === 'examquestions' && examQuestions.length > 0 && <span style={{ marginLeft: '0.4rem', fontSize: '0.7rem' }}>({examQuestions.length})</span>}
            </button>
          ))}
        </div>
          ...
          {activeTab === 'mcqs' && (
            mcqs.length > 0 ? (
              currentQ < mcqs.length ? (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    <p style={{ fontSize: '0.78rem', color: '#5a5a4a', fontFamily: 'monospace' }}>Question {currentQ + 1} of {mcqs.length}</p>
                  ...
                  <p style={{ fontSize: '1rem', fontWeight: 700, lineHeight: 1.6, marginBottom: '1.5rem' }}>{mcqs[currentQ].question}</p>
                  ...
                  {selected !== null && (
                    <div>
                      <div style={{ background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '10px', padding: '1rem', marginBottom: '1rem' }}>
                        <p style={{ fontSize: '0.8rem', color: '#c0c0b0', lineHeight: 1.6 }}>💡 {mcqs[currentQ].explanation}</p>
                      </div>
                      <button onClick={nextQuestion} style={{ width: '100%', padding: '0.9rem', borderRadius: '10px', border: 'none', background: '#f59e0b', color: '#0d0d0a', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                        Next Question →
                      </button>
                    </div>
                  )}
                ...
                <div style={{ textAlign: 'center', padding: '2rem 0' }}>
                  ...
                  <p style={{ fontSize: '1rem', color: '#f59e0b', fontWeight: 700, marginBottom: '0.5rem' }}>Score: {score}/{mcqs.length}</p>
            ...
            examQuestions.length > 0 ? (
              <div>
                <div style={{ marginBottom: '1.5rem' }}>
                  <p style={{ fontSize: '0.78rem', color: '#5a5a4a', fontFamily: 'monospace', marginBottom: '0.25rem' }}>
                    {examQuestions.length} likely exam questions generated
                  </p>
                  <p style={{ fontSize: '0.78rem', color: '#3a3a30' }}>Practice writing full answers to these</p>
                </div>
